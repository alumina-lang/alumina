...
Missing:
- AST node coverage audit done: aluminac's `ast.alu` `ExprTag` covers every alumina-boot `ExprKind` variant (with renames — e.g. boot's `Lit` is split into `IntLit/FloatLit/BoolLit/StringLit/CharLit/NullLit`; boot's `Static/Const/Fn/EnumValue` map to `StaticRef/.../FnRef/EnumRef`; boot's `Macro/MacroInvocation` collapse into `MacroInvocation`). Boot's `Yield` is out-of-scope (coroutines). Boot's `Tag` is an internal wrapper not used outside boot. Boot's `AssignOp` is folded into aluminac's `Assign` with an op tag. The `Ty` enum is similar — aluminac has `DerefOf` and `Dyn` that boot expresses with other variants. No gaps blocking compilation.
