...
3. **Reflection lang items** (~~`enum_variant_new`~~, ~~`field_descriptor_new` / `field_descriptor_new_unnamed`~~, `type_descriptor_new`, `format_arg`). Wiring them lets the four stub intrinsics (`enum_variants`, `fields`, `vtable`, `attributed`) build their array-of-call results directly. **No const_alloc dependency** — alumina-boot's implementations are plain `array_of(...)` expressions (verified against `src/alumina-boot/src/ir/mono/intrinsics.rs`'s `intr_enum_variants`). Earlier framings in this file that linked these to `const_alloc` were wrong. **Status (2026-05-11): `enum_variant_new` + `enum_variants` + `field_descriptor_new` + `fields` intrinsic done** — both lang-item-tagged constructors (in `sysroot-aluminac/std/typing/mod.alu`) and the `lower_enum_variants` / `lower_fields` impls in `mono/intrinsics.alu` are wired and tested. Remaining: `type_descriptor_new` and `format_arg` (used internally by sysroot's typing.alu / fmt.alu — wiring needs unifying those files which is a larger slice), and the `vtable` / `attributed` intrinsics (separate work).
...
5. **Macro-internal `use` doesn't reach expansion site.** A macro body like `macro foo($a) { use std::fmt::debug; $a.debug() }` fails at `$a.debug()` with "could not resolve method 'debug' on type ''" — the `use` doesn't make `debug` visible at the expansion site. Workaround: use the fully-qualified path `std::fmt::debug(_a)` instead of UFCS. Real fix would be honouring the macro-internal `use` during expansion. **Discovered 2026-05-11 while trying to align `assert_eq!` between sysroot-aluminac and sysroot** — sysroot's version uses `_a.debug()` to format unfmt types like `Instant`, but porting it to sysroot-aluminac requires either this fix or rewriting all assert_eq! variants to fully-qualified paths (the latter then segfaulted under test-std-aluminac, suggesting another tangled issue downstream).
...
- [PARTIAL] **Reflection lang items** (`format_arg`, ~~`enum_variant_new`~~, `field_descriptor_new`, `field_descriptor_new_unnamed`, `type_descriptor_new`). Required by `enum_variants` / `fields` intrinsics. As of 2026-05-11, `enum_variant_new` is wired (sysroot-aluminac has the lang-item-tagged constructor, intrinsics.alu's `lower_enum_variants` calls it). `field_descriptor_new` / `field_descriptor_new_unnamed` are declared in `sysroot-aluminac/std/typing/mod.alu` but `lower_fields` still returns an empty stub — needs the per-field layout computation (`compute_type_size` walk) similar to alumina-boot's `intr_fields`. `format_arg` and `type_descriptor_new` are declared sysroot-side but not yet exercised through aluminac-specific code.
...
- [PARTIAL] **`std/time.alu`** — unification attempted 2026-05-11 then reverted (commits cb7f4380 / 7ec80c85). Switching `sysroot-aluminac/std/time.alu` to sysroot's version compiles cleanly for the basic API but breaks `make test-std-aluminac` because aluminac's `test_cases!()` builtin discovers and mono's the embedded `tests` module. The two failures, narrowed down via standalone repros:
  1. `test_monotonish` uses `thread::sleep` which sysroot-aluminac doesn't have. Gating the test under `#[cfg(threading)]` upstream is the simplest fix.
  2. `test_artithmetic` ends with `assert_eq!(start1, start)` where both are `Instant` values. **`Instant` has no `fmt` impl** in sysroot — sysroot's `assert_eq!` works around this by calling `lhs.debug()` / `rhs.debug()` (DebugAdapter has graceful fallback for unfmt types), but **sysroot-aluminac's `assert_eq!` calls `fmt` directly** via `eprintln!("... {} != {}", _a, _b)`. So the divergent macro expansion is the actual blocker, not a Duration fmt issue (verified: `assert_eq!` on Duration alone works fine).
  Path forward: align `assert_eq!`'s macro between the two sysroots — i.e. unify `std/mod.alu`'s `assert_eq` to use the `.debug()`-based path. That in turn requires unifying `std/panicking.alu` (catch_panic infrastructure, separate slice with its own Formatter-conformance loop blocker) or adding a `panic_with_hook` shim alongside `panic_impl` in the unified mod.alu.
