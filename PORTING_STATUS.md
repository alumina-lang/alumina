...
- [PARTIAL] **`builtin_never`, `builtin_u128`, `builtin_i128`.** `builtin_u128` and `builtin_i128` lang stubs added in `sysroot-aluminac/std/builtins.alu`; `link_builtin_lang_items` already wires them via the existing `u128`/`i128` BuiltinType entries. Verified by `tests/aluminac/u128_i128_basic.alu` (128-bit arithmetic, lang-item-linked methods, casts).
  Missing:
