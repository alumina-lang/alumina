...
- [PARTIAL] **Bit-twiddling intrinsics: `count_ones` / `count_zeros` / `leading_zeros` / `trailing_zeros` / `swap_bytes`.** All five are now methods on `u8/u16/u32/u64/usize` and `i8/i16/i32/i64/isize` in sysroot-aluminac, lowered via `intrinsics::llvm("llvm.ctpop"/...)`. The LlvmIntrinsic codegen path special-cases `llvm.ctlz`/`llvm.cttz`: truncs the `is_zero_undef` flag from i8 to i1 and forces the overload list to a single iN entry (the bool is fixed in the intrinsic signature, not part of the suffix). 8-bit `swap_bytes` is identity since LLVM `bswap` requires width ≥ 16. Verified by `tests/aluminac/bit_twiddle.alu`.
  Missing:
