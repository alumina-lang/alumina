...
- [PARTIAL] **Custom attributes (`Attribute::Custom`).** AttrTag::Custom (= 26) added. pass1.alu's catch-all now stores unknown `#[name(...)]` attributes as `Custom` carrying the literal name (skipping known diagnostics-only names: `allow`, `docs`, `deny`, `warn`, `cfg_attr`). `has_attribute<T>(name)` looks up Custom attributes on `Fn`-typed T's via `fn_defs`. Verified by `tests/aluminac/custom_attr.alu`.
  Missing:
  - Struct / enum types: aluminac doesn't yet propagate attributes onto IrStructRef / IrEnumRef so `has_attribute::<MyStruct>("foo")` returns false. Plumbing those is the rest of the work.
  - The `attributed(...)` intrinsic that returns a slice of items by attribute name. Depends on global custom-attribute index + reflection lang items.
...
- [PARTIAL] **Float classification (`is_finite` / `is_nan` / `is_infinite` / `is_normal`) at const time.** Smoke-checked: `const X: bool = (1f64).is_finite()` and `const Y: bool = (1f64/0f64).is_infinite()` const-evaluate correctly. The implementations in sysroot-aluminac/std/builtins.alu use bit-pattern checks (no codegen_func dependency), so the const evaluator handles them. `is_normal` and corner cases (subnormals, signed zero) not separately stressed.
...
- [PARTIAL] **Intrinsic: `has_attribute`.** Recognized in `mono/intrinsics.alu`. For Fn-typed `T`, looks up `def.attributes` via `fn_defs` and checks for `AttrTag::Custom` matching the literal name argument (verified by `tests/aluminac/custom_attr.alu`). For Struct / Enum types, still returns false until those carry attributes (partial — see Custom-attribute entry in Language features).
...
- [DONE] **Range constructor lang items** (`range_full_new`, `range_from_new`, `range_to_new`, `range_to_inclusive_new`, `range_new`, `range_inclusive_new`). Functionally covered by aluminac's inline range-literal lowering, which produces the same struct lit shape that the lang-item functions would. The lang-item versions in sysroot are `#[inline(ir)]` thin wrappers around the same struct construction, so observable behavior is identical.
