...
3. **Reflection lang items** (~~`enum_variant_new`~~, `field_descriptor_new` / `field_descriptor_new_unnamed`, `type_descriptor_new`, `format_arg`). Wiring them lets the four stub intrinsics (`enum_variants`, `fields`, `vtable`, `attributed`) build their array-of-call results directly. **No const_alloc dependency** — alumina-boot's implementations are plain `array_of(...)` expressions (verified against `src/alumina-boot/src/ir/mono/intrinsics.rs`'s `intr_enum_variants`). Earlier framings in this file that linked these to `const_alloc` were wrong. **Status (2026-05-11): `enum_variant_new` + `enum_variants` intrinsic done** — both the lang-item-tagged constructor (in `sysroot-aluminac/std/typing/mod.alu`'s `internal::make_enum_variant`) and the `lower_enum_variants` impl in `mono/intrinsics.alu` are wired and tested. The `field_descriptor_new` lang items are declared but their `fields` intrinsic isn't yet using them (next slice).
