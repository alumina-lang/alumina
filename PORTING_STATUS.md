...
- [PARTIAL] **`std/result.alu`** — Same shape as Option. Has `is_ok`/`is_err`/`unwrap`/`map`/`map_err`/`and_then`/`or_else`/`transpose`/`equals`/`fmt`. Newly added `mixin Equatable<Result<T, E>>`, `hash<...>` + `mixin Hashable<Result<T, E>, H>`, plus `AnyResult` type alias (`tests/aluminac/option_result_equatable.alu`, `option_result_hash.alu`, `option_result_any.alu`). `fmt::debug<T>` + `DebugAdapter` stub now landed (defers to T's own fmt for Formattable, type name otherwise).
  ...
  - Result::unwrap end-to-end against the unified sysroot still fails, but at a *different* point — vtable build for builtin types (e.g. `i32` impl'ing `Formattable`) hits the fallback `method_type_args = &[]` arm because i32 isn't a struct, so `fmt`'s `F` placeholder is unbound and `format_integer`'s body resolves `fmt: &mut F` to `&mut void`. The error chain is now `unwrap_panic_err → dyn_format_arg → fmt → format_integer` with "could not resolve method 'write_byte' on type ''" at fmt/mod.alu:348:21. Separate slice: extend the vtable build's method_type_args calculation in `mono/lower.alu` (around line 3821) to handle the impl-non-struct + protocol-with-Self+F case (currently all four branches require `combined_total == fn_def.generic_params.len()` or equivalent, which doesn't hold here).
...
- [PARTIAL] **Aluminac compiles dyn-free programs against `sysroot/`.** The slices in this branch (typeop dispatch, slice pseudo-fields, range type inference, fn-item type resolution, generic-fn skip-on-export, etc.) collectively let aluminac swallow non-trivial code against the unified sysroot — verified by `tests/aluminac/unified_sysroot_basic.alu`. The test exercises generics, Ordering, comparison-operator overload dispatch on a user struct, and works as a regression guard.
  ...
  - `Result::unwrap` against `--sysroot sysroot` advanced past the type_map pollution (fixed in this commit) but now fails on a separate vtable-build gap: builtin types (e.g. i32) impl'ing `Formattable<Self, F>` fall through to `method_type_args = &[]` in the dyn vtable builder, leaving `F` unbound when `fmt` is mono'd. The fix shape is documented next to the result.alu entry. Tracked.
