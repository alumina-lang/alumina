...
- [PARTIAL] **`std/option.alu`** — Option already has the bulk of methods (`is_some`/`is_none`/`unwrap`/`map`/`and_then`/`or_else`/`flatten`/`zip`/`as_ptr`/`replace`/`transpose`/`equals`/`fmt`). Newly added `Option::iter() -> OnceIterator<T>` (`tests/aluminac/option_iter.alu`) and `mixin<T: Equatable<T>> Equatable<Option<T>>` so options satisfy `T: Equatable<T>`-bounded generics (verified by `tests/aluminac/option_result_equatable.alu`).
  ...
  Literal unification attempted in this session — the file compiles cleanly under aluminac with all Option-specific tests passing (28 of 30). But the embedded test_range / test_range_inclusive in std/range.alu fail with "some(5) != some(5)" / "some(6) != some(6)" — `_a != _b` returns true between two equal Option<usize> values when sysroot's option.alu definition is used. Likely a dispatch interaction between the mixin'd not_equals and the impl-level `equals<T: Equatable<T>>` whose method-level T shadows the impl T. sysroot-aluminac's existing impl avoids the shadow via a different impl-block structure, which is why it works. Tracked as a separate dispatch gap; unification of option.alu is gated on resolving it.
