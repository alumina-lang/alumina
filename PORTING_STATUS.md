...
The remaining compiler-side work clusters into four real items. Closing these unblocks most of the stdlib `[TODO]`s en masse; chasing stdlib files one at a time without fixing these is depth-first work against shallow leaves.

1. **Protocol-method Self uniformity.** `def.generic_params` shape is asymmetric between `build_protocol` (Self included) and `build_function` (Self not included). Real refactor in `parser/pass2.alu` + dispatch sites. See PARTIAL **`dyn` / dynamic dispatch** entry below for diagnosis. Blocks `Result::unwrap` end-to-end and probably several near-cases the existing dyn tests don't exercise.
...
- [PARTIAL] **`dyn` / dynamic dispatch.** Real dispatch wired end-to-end for single- and multi-protocol bounds. `&T as &dyn Proto` now emits a private const global vtable (`[fn(); N]`) populated with `T`'s typed method pointers (cast to opaque `fn()`), and threads its address into the dyn fat pointer's `_vtable` slot. Method calls on dyn values load `_vtable[idx]`, cast to the typed fn pointer, and invoke with `(_ptr, args...)`. Multi-protocol bounds `&dyn (A + B + ...)` are captured (parser iterates `inner` fields, synthesizes a TupleTy; resolve_type unpacks back into the dyn struct's `Protos` type-arg). Verified by `tests/aluminac/dyn_dispatch.alu` (single-method + multi-method protocols, plus a `&dyn (A + B)` bound).
  ...
  **Structural gap exposed 2026-05-11** — the "Multi-type-arg protocols work" claim below was based on tests that only exercised user-struct impls. For *protocol-default* methods on builtins (e.g. `i32` getting `fmt` through the `IntegerFormattable` mixin), the vtable builder hits "could not resolve method 'write_byte' on type ''" at `sysroot/std/fmt/mod.alu:348:21`. Root cause is asymmetric `def.generic_params` shape, not a missing vtable-build branch:

  - `build_protocol` (in `parser/pass2.alu:1031`) prepends the protocol's *full* generic_params (Self included) to each method → `[Self_id, F_id, ...]`.
  - `build_function` for impl methods (`parser/pass2.alu:261`) prepends only `impl_generic_param_ids` (Self NOT included — Self is implicit from the impl type) → `[T_impl_id, F_method_id]`.

  The vtable build in `mono/lower.alu` (around 3821) tries to reconcile these with branches matching `len(generic_params)` against various combinations of `impl_args.len()` and `proto_non_self_count`. Adding a fifth branch for the protocol-default-on-builtin case doesn't fix it (verified mid-session, reverted). The right fix is **structural uniformity**: pick one model (alumina-boot's model is "Self always in placeholders") and make pass2 + every dispatch site consume `def.generic_params` the same way regardless of whether the method was built by build_protocol or build_function. After that, the vtable-build branches collapse to one rule.

  This is the actual blocker behind today's `Result::unwrap` end-to-end failure and explains why I keep finding "the vtable build needs another branch" — the same asymmetry surfaces in different shapes.
...
- [PARTIAL] **`std/result.alu`** — Same shape as Option. Has `is_ok`/`is_err`/`unwrap`/`map`/`map_err`/`and_then`/`or_else`/`transpose`/`equals`/`fmt`. Newly added `mixin Equatable<Result<T, E>>`, `hash<...>` + `mixin Hashable<Result<T, E>, H>`, plus `AnyResult` type alias (`tests/aluminac/option_result_equatable.alu`, `option_result_hash.alu`, `option_result_any.alu`). `fmt::debug<T>` + `DebugAdapter` stub now landed (defers to T's own fmt for Formattable, type name otherwise).
  ...
  - The type_map pollution gap is fixed: `mono_function` now snapshots prior `type_map` entries for its own generic-params at entry and restores the ones that were *bound* before on exit (entries that were unbound stay set, preserving the leaky-context behavior the dispatcher paths rely on for filling in method-only generics). And nested fns now key their cache against the outer mono's cache_key, so `unwrap_panic_err` (the nested helper inside `Result::unwrap`) gets a fresh mono per outer instantiation instead of reusing the first one. Together: the original "expected 'i32', got 'Error'" at result.alu:247:30 is gone. Verified by `tests/aluminac/recursive_mono_type_map.alu` (the type_map snapshot) and `tests/aluminac/nested_fn_outer_generics.alu` (nested-fn cache keying). Both tests run under `--sysroot sysroot-aluminac` and discriminate against baseline.
  - Result::unwrap end-to-end against the unified sysroot still fails, but at a *different* point — error chain `unwrap_panic_err → dyn_format_arg → fmt → format_integer` with "could not resolve method 'write_byte' on type ''" at fmt/mod.alu:348:21. **Diagnosis (2026-05-11):** symptom is `fmt`'s `F` placeholder unbound after mono. Initial guess "vtable build needs another branch for impl-non-struct" — verified mid-session by adding that branch — does **not** fix it. Real cause is the protocol-method Self uniformity gap (see top of Language features section). The fix path is upstream of the vtable build; this entry will close together with that one.
...
- [PARTIAL] **Aluminac compiles dyn-free programs against `sysroot/`.** The slices in this branch (typeop dispatch, slice pseudo-fields, range type inference, fn-item type resolution, generic-fn skip-on-export, etc.) collectively let aluminac swallow non-trivial code against the unified sysroot — verified by `tests/aluminac/unified_sysroot_basic.alu`. The test exercises generics, Ordering, comparison-operator overload dispatch on a user struct, and works as a regression guard.
  ...
  Missing:
  - Stdlib code that uses `dyn` directly (regex internal DFA, runtime backtrace, typing reflection, io/fs Read/Write protocols) — still unverified end-to-end. Likely affected by the **Protocol-method Self uniformity** structural gap (see "Highest-leverage remaining work" at the top of Language features).
  ...
  - `Result::unwrap` against `--sysroot sysroot` advanced past the type_map pollution (fixed 2026-05-11) but still fails. The remaining symptom — vtable build leaving `F` unbound when mono'ing `fmt` on builtin `i32` — is a manifestation of the Self-uniformity gap, not a separate vtable-build branch to add. Verified mid-session that adding a fifth branch doesn't fix it.
