...
- [PARTIAL] **Bit-twiddling intrinsics: `count_ones` / `count_zeros` / `leading_zeros` / `trailing_zeros` / `swap_bytes`.** `count_ones` / `count_zeros` / `swap_bytes` are now methods on `u8/u16/u32/u64/usize` and `i8/i16/i32/i64/isize` in sysroot-aluminac, lowered via `intrinsics::llvm("llvm.ctpop", ...)` and `"llvm.bswap"` (8-bit `swap_bytes` is identity since LLVM bswap requires width ≥ 16). Verified by `tests/aluminac/bit_twiddle.alu`.
  Missing:
  - `leading_zeros` / `trailing_zeros` need `llvm.ctlz` / `llvm.cttz`, which take a fixed `i1` `is_zero_undef` arg. The current LlvmIntrinsic codegen path can only special-case `llvm.expect.i1` (which has all-i1 args); `ctlz`/`cttz` mix iN with a fixed i1 and would also need the overload-types deduction to drop the i1, which the current dedup-on-unique-types heuristic gets wrong.
  - 128-bit width: aluminac's u128/i128 path is itself PARTIAL, so the methods are not added there yet.
  - Const-evaluation: `const X: u32 = (0xAAu8).count_ones()` falls back to runtime — const_eval doesn't currently interpret LlvmIntrinsic nodes.
