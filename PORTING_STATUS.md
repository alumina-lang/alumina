...
- [TODO] **LLVM phi pattern crashes GlobalISel under the panic chain.** Mono of `Option::unwrap` against `--sysroot sysroot` produces LLVM IR that segfaults `libLLVM-14.so`'s `IRTranslator::finishPendingPhis` — the underlying check is `MachineBasicBlock::isPredecessor`, suggesting a phi's incoming-block list disagrees with the machine-IR predecessor set. The textual IR (`--emit-llvm`) emits cleanly and looks well-formed, so the discrepancy is between LLVM-IR-level CFG and the GlobalISel machine-IR projection. Likely an unreachable predecessor pattern or a phi node in a block reached only via a defaulted switch (similar to the earlier `merge_bb` self-predecessor bug, but the `unreachable` synthetic-default fix doesn't cover this case). Blocks the unified-sysroot panic-chain end-to-end test.
...
- [PARTIAL] **`std/result.alu`** — Same shape as Option. Has `is_ok`/`is_err`/`unwrap`/`map`/`map_err`/`and_then`/`or_else`/`transpose`/`equals`/`fmt`. Newly added `mixin Equatable<Result<T, E>>`, `hash<...>` + `mixin Hashable<Result<T, E>, H>`, plus `AnyResult` type alias (`tests/aluminac/option_result_equatable.alu`, `option_result_hash.alu`, `option_result_any.alu`). `fmt::debug<T>` + `DebugAdapter` stub now landed (defers to T's own fmt for Formattable, type name otherwise).
  ...
  - At least three intertwined gaps: (1) `panicking::internal::PanicFormatter` is missing from sysroot-aluminac (sysroot's 472-line dyn-based panicking vs aluminac's 73-line stub); (2) even with an empty PanicFormatter stub, the `unwrap_panic_err(self._inner.err)` call type-checks as "expected 'Error', got 'i32'" — likely because mono of the nested fn pollutes `type_map` for the outer impl's E, mistyping the field access on re-lower; (3) sysroot uses `&dyn fmt::Formattable<Self, F>` for the panic-args slice, which needs full dyn support. Adding both `write_str` and `write_byte` methods to a struct inside aluminac's own panicking module also segfaults the self-build (likely a Formatter-conformance loop), so even shimming PanicFormatter isn't trivial. Result.alu unification is gated on either porting all three of these or reworking the panic path in sysroot to avoid them.
...
- [PARTIAL] **Aluminac compiles dyn-free programs against `sysroot/`.** The slices in this branch (typeop dispatch, slice pseudo-fields, range type inference, fn-item type resolution, generic-fn skip-on-export, etc.) collectively let aluminac swallow non-trivial code against the unified sysroot — verified by `tests/aluminac/unified_sysroot_basic.alu`. The test exercises generics, Ordering, comparison-operator overload dispatch on a user struct, and works as a regression guard.
  ...
  Mono of `Option::unwrap` (and the rest of the panic chain — `Result::unwrap`, `panic!`, `dyn Formattable`-based `const_panic_impl`) now succeeds against `--sysroot sysroot`. The wins required threading the protocol's non-Self type-args through `IrTyTag::Protocol`, the dyn vtable build assembling `[impl_struct.type_args..., proto.non_self_args...]` for impl methods, single-proto direct-Placeholder unification of `&dyn Proto<...>` in expected-return-type inference, and propagating expected element type into array-literal lowering (the panic path expands `format_args!` into an array of `dyn_format_arg(&...)` calls whose F must be solved from the array's expected type).

  **Codegen blocker remains:** the resulting LLVM IR for the panic chain segfaults LLVM 14's GlobalISel pipeline at `IRTranslator::finishPendingPhis` → `MachineBasicBlock::isPredecessor`. Mono completes cleanly and `--emit-llvm` produces the IR file, but invoking the backend (which is what `aluminac -o foo.bin` does at the very end) crashes inside libLLVM. The IR's phi nodes look syntactically well-formed and the same construction works for sysroot-aluminac shapes, so the trigger is some specific phi pattern under GlobalISel — likely an unreachable predecessor or a phi reaching a block that's marked but not reachable in the machine-IR view. Tracked as a follow-up.
