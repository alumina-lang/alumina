...
- [PARTIAL] **Custom attributes (`Attribute::Custom`).** AttrTag::Custom (= 26) added. pass1.alu's catch-all stores unknown `#[name(...)]` attributes as `Custom` carrying the literal name (skipping `allow`, `docs`, `deny`, `warn`, `cfg_attr`). `has_attribute<T>(name)` works on Fn-typed T's (via `fn_defs`) and on Struct-typed T's (via `IrStructRef.attributes`, populated at mono time). Verified by `tests/aluminac/custom_attr.alu`.
  Missing:
  - Enum types: `IrEnumRef` doesn't yet carry attributes. Symmetric to the struct path; trivial to extend when needed.
...
- [PARTIAL] **Intrinsic: `has_attribute`.** Recognized in `mono/intrinsics.alu`. For Fn-typed `T`, consults `fn_defs[id].attributes`. For Struct-typed `T`, consults `IrStructRef.attributes` (populated at mono time from `def.attributes`). Both verified by `tests/aluminac/custom_attr.alu`. Enum types still return false (`IrEnumRef` doesn't carry attributes yet — trivial extension).
