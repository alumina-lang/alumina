...
- [PARTIAL] **`std/result.alu`** — Same shape as Option. Has `is_ok`/`is_err`/`unwrap`/`map`/`map_err`/`and_then`/`or_else`/`transpose`/`equals`/`fmt`. Newly added `mixin Equatable<Result<T, E>>`, `hash<...>` + `mixin Hashable<Result<T, E>, H>`, plus `AnyResult` type alias (`tests/aluminac/option_result_equatable.alu`, `option_result_hash.alu`, `option_result_any.alu`). `fmt::debug<T>` + `DebugAdapter` stub now landed (defers to T's own fmt for Formattable, type name otherwise).
  ...
  - The when-discriminator `when err is fmt::Formattable<E, panicking::internal::PanicFormatter>` references `panicking::internal::PanicFormatter`, which doesn't exist in `sysroot-aluminac/std/panicking.alu` (sysroot has it; aluminac's panicking is a 73-line stub vs sysroot's 472-line dyn-based version). Without PanicFormatter, the unresolved path appears to misparse and the type-mismatch surfaces as "expected 'Error', got 'i32'" at the `unwrap_panic_err(self._inner.err)` call. Real fix: port `panicking::internal::PanicFormatter` (and the surrounding `panic_impl` infrastructure), which is gated on `dyn` since `PanicFormatter` is used through `&dyn fmt::Formattable<...>`.
